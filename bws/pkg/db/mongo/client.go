...
var (
	_clientMap = map[string]*mongo.Client{}
	_mu        sync.Mutex
)
...
func GetMongoClient(opts ...ClientOption) (c *mongo.Client, err error) {
	...
	if _opts.Host == "" {
		...
		if _opts.Host == "" {
			_opts.Host = DEFAULT_MONGO_HOST
		}
	}
	if _opts.Port == "" {
		_opts.Port = (viper.GetString("mongo.port"))
		if _opts.Port == "" {
			_opts.Port = (DEFAULT_MONGO_PORT)
		}
	}
	if _opts.Db == "" {
		_opts.Db = (viper.GetString("mongo.db"))
		if _opts.Db == "" {
			_opts.Db = (DEFAULT_MONGO_DB)
		}
	}
	...
	if _opts.AuthSource == "" {
		...
		if _opts.AuthSource == "" {
			_opts.AuthSource = (DEFAULT_MONGO_AUTH_SOURCE)
		}
	}
	...
	_optsKeyBytes, err := json.Marshal(opts)
	if err != nil {
